...
class Whmcs_Domain_Checker {
	 ...
	 * @var      string
	 */
	protected $plugin_slug = 'whmcs-domain-checker';
	/**
	 * @var      object
	 */
	...
	public function get_post_meta($id, $key = null, $single = false){
		...
		if(!empty($key)){
			...
			if(file_exists(self::get_path( __FILE__ ) .'configs/fieldgroups-whmcs_domain_checker.php')){
				include self::get_path( __FILE__ ) .'configs/fieldgroups-whmcs_domain_checker.php';		
			}else{
				return;
			}
	...
	function save_post_metaboxes($pid, $post){
		...
		if(!isset($_POST['whmcs_domain_checker_metabox']) || !isset($_POST['whmcs_domain_checker_metabox_prefix'])){return;}


		if(!wp_verify_nonce($_POST['whmcs_domain_checker_metabox'], plugin_basename(__FILE__))){
			return $post->ID;
		}
		...
		foreach( $_POST['whmcs_domain_checker_metabox_prefix'] as $prefix ){
			if(!isset($_POST[$prefix])){continue;}

			if($_POST['whmcs_domain_checker_storage'] == 'single'){
			...
			//foreach($_POST['whmcs_domain_checker'] as $field=>$data){
			/*
			*/
			//}
		}
	...
	public function render_element($atts, $content, $slug, $head = false) {
		...
		if(!empty($head)){
			$instanceID = $this->element_instance_id('whmcs_domain_checker'.$slug, 'header');
		}else{
			$instanceID = $this->element_instance_id('whmcs_domain_checker'.$slug, 'footer');
		}
